...
const getArticleContents = async (article) => {
  ...
  await axios
    ...
    .then(response => {
      ...
      // SINCE GOOGLE NEWS API SOMETIMES FAILS TO LOAD THUMBNAIL/FAVICON, USE LOGIC TO CHECK AND ADD WITH EXT. NEWS
      ...
      if (article.favicon === 'https://res.cloudinary.com/dyexzgvkb/image/upload/v1626528473/favicon_acrnzm.png') {
...
export default function VirtualizedList(props) {
  ...
  const renderRow = (props) => {
    ...
    const dualClick = async () => {
      await getArticleContents(articles[index]);
      handleOpen()
      open === false ?
        setNewNum(index) :
        newNum < articles.length - 1 ?
          setNewNum(newNum + 1) :
          null;
    };

    const nextArticle = event => {
      if (event.key === "ArrowRight" && newNum < articles.length - 1) setNewNum(newNum + 1);
      if (event.key === "ArrowLeft" && newNum > 0) setNewNum(newNum - 1);
    };

    return (
      <ListItem className="zoom" sx={{marginLeft: "-5.5px", borderColor: "white"}} style={style} key={index}
                component="div" disablePadding alignItems="center">

        <ListItemButton onClick={dualClick} sx={{
          '&:hover': {
            ...
            <Box sx={style2}>
              ...
              <Box sx={{display: "flex", justifyContent: "space-between"}}>
                ...
                <Typography sx={{
                  ...
                  textAlign: "center",
                  fontWeight: 600,
                  fontFamily: "Helvetica Neue,Helvetica,Arial,Utkal,sans-serif"
                }}>"{articles[newNum].description}"
                </Typography>
              </Box>
              ...
              <Typography id="modal-modal-description"
                sx={{mt: 5, fontFamily: "Helvetica Neue,Helvetica,Arial,Utkal,sans-serif", fontWeight: 380}}>
                {articles[newNum].body}
              </Typography>
